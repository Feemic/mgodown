...
func codeSpan(p *Parser, data []byte, offset int) (int, ast.Node) {
	...
	if fBegin != fEnd {
		if (data[fBegin] == '$' && data[fEnd-1] == '$'){ //feemic add
			// create inline math node
			math := &ast.Math{}
			math.Literal = data[fBegin+1:fEnd-1]
			return end, math
		}
		code := &ast.Code{}
...
func helperDoubleEmphasis(p *Parser, data []byte, c byte) (int, ast.Node) {
	...
	for i < len(data) {
		...
		if i+1 < len(data) && data[i] == c && data[i+1] == c && i > 0  { //feemic fixed:&& !isSpace(data[i-1])
