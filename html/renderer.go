...
func NewRenderer(opts RendererOptions) *Renderer {
	...
	if opts.Generator == "" {
		opts.Generator = `  <meta name="GENERATOR" content="github.com/Feemic/mgodown markdown processor for Go`
	}
...
func (r *Renderer) codeBlock(w io.Writer, codeBlock *ast.CodeBlock) {
	...
	if string(codeBlock.Info) == "math" && (r.opts.Flags&MathCode != 0 ){ //handle math block
		lines := bytes.Split(codeBlock.Literal, []byte("\n"))
		for _, line := range(lines){
			if len(line) == 0 {
				continue
			}
			r.cr(w)
			r.outs(w, `<p><span class="math display">\[`)
			EscapeHTML(w, line)
			r.outs(w, `\]</span></p>`)
		}
		return
